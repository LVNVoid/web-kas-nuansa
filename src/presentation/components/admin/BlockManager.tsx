...
import { ResidentBlock } from "@/core/entities/block.entity";

export type ResidentBlockData = ResidentBlock;

interface BlockManagerProps {
  initialBlocks: ResidentBlock[];
}

export function BlockManager({ initialBlocks }: BlockManagerProps) {
  const [search, setSearch] = useState("");
  const [modalOpen, setModalOpen] = useState(false);
  const [editingBlock, setEditingBlock] = useState<ResidentBlock | null>(null);
  ...
  const handleOpenEdit = (block: ResidentBlock) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (!res.success) {
        const err = res.error || "Gagal menyimpan data.";
        setErrorMsg(err);
        toast.error(err);
      } else {
        toast.success(
          editingBlock
            ? `Blok ${editingBlock.blockName} berhasil diperbarui`
  ...
  const handleDelete = (id: string, blockName: string) => {
    ...
    startTransition(async () => {
      ...
      if (!res.success) {
        toast.error(res.error || "Gagal menghapus blok.");
      } else {
        toast.success(`Blok ${blockName} berhasil dihapus`);
      }
    });
  };
