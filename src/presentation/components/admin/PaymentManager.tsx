...
import { BlockPaymentRow } from "@/core/entities/payment.entity";

export type { PeriodData, BlockPaymentRow };
...
export function PaymentManager({
  ...
  const handleQuickToggle = (row: BlockPaymentRow) => {
    ...
    startTransition(async () => {
      ...
      if (!res.success) {
        toast.error(res.error || "Gagal mengubah status bayar.");
      } else {
        toast.success(
          nextStatus
            ? `Setoran ${row.blockName} ditandai LUNAS`
            : `Setoran ${row.blockName} dibatalkan (BELUM)`
        );
      }
    });
  };
  ...
  const handleSavePaymentDetail = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (!res.success) {
        const err = res.error || "Gagal menyimpan rincian pembayaran.";
        setErrorMsg(err);
        toast.error(err);
      } else {
        toast.success(`Rincian setoran ${editingRow?.blockName} berhasil disimpan`);
        setEditingRow(null);
      }
    });
  };
  ...
  const handleCreatePeriod = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (!res.success) {
        const err = res.error || "Gagal membuat periode.";
        setErrorMsg(err);
        toast.error(err);
      } else {
        toast.success("Periode pembukuan baru berhasil dibuat");
